...
이 게시물에서는 어떤 개발 환경이든 사용할 수 있는 Nix와 Home Manager 구성을 설정할 것입니다.
최종적으로 다음과 같은 목표를 달성하고자 합니다:

1. 다른 컴으로 쉽게 이전할 수 있는 Nix와 Home Manager 설정 구성.
...
설치가 (다소) 일반화되도록 하려면 서로 다른 기계 간에 구성을 공유하는 방법이 필요합니다.
...
또한, https://github.com/numtide/flake-utils를 내 flake에 가져왔습니다. 이를 통해 각각의 사용하는 시스템마다 flake 구성을 정의할 수 있습니다.
...
그러나 이러한 구성을 모든 기계에서 작동하도록 만들고 싶으므로, home-manager 활성화를 위한
다른 스크립트를 제공해 봅시다.
